export default {
	...
	withdrawLocal(web3_, tx_, exchangeContract_, exchangeAddress_, account_, pkey_, gasPrice_, amount_, value_) {
		const vm = this;
		const withdrawFunction = exchangeContract_.methods.withdraw(amount_);
		const withdrawAbi = withdrawFunction.encodeABI();
		vm.signAndSend(tx_, withdrawFunction, withdrawAbi, exchangeAddress_, account_, pkey_, gasPrice_, value_);
	},
	...
	withdrawTokenLocal(web3_, tx_, exchangeContract_, tokenContract_, exchangeAddress_, tokenAddress_, account_, pkey_, gasPrice_, amount_, value_) {
		const vm = this;
		const withdrawFunction = exchangeContract_.methods.withdrawToken(tokenAddress_, amount_);
		const withdrawAbi = withdrawFunction.encodeABI();
		vm.signAndSend(tx_, withdrawFunction, withdrawAbi, exchangeAddress_, account_, pkey_, gasPrice_, value_, res => console.log(res));
	},
